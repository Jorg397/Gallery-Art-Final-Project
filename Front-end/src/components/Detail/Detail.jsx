		...
		<div className={s.containerG}>
			...
			<div className={s.containerRoute}>
				<div className={s.containerRoute__title}>
					<div>
						<NavLink to={"/home"} className={`${s.route}`}>
							Home {">"}{" "}
						</NavLink>
						<NavLink to={"/gallery"} className={`${s.route}`}>
							Galería {">"}{" "}
						</NavLink>
						<NavLink to={"#"} className={`${s.route}`}>
							{resp?.name} {">"}{" "}
						</NavLink>
					</div>
				</div>
			...
			<div className={`${s.principalContainer} mt-10`}>
				<section>
					<div
						className={`pt-32 flex justify-around pb-10 ${s.secondContainer}`}>
						<div className={`${s.containerIMG} mt-32`}>
							<div>
								<img
									src={`${resp?.image}`}
									alt='picture'
						...
						<div
							className={`${s.detailsContainer} flex flex-col -mt-32`}>
							...
							<div
								...
								<button
									className={`${s.button}`}
									onClick={() =>
										handleAddToCart(resp?.idProduct)
									}>
									Agregar al carrito
								</button>
							</div>
							...
							<p className='text-xl text-white mx-5 my-5'>
								* El pago del envio sera cubierto por cada
								comprador de las cuales se referenciara el costo
								con la direccion de envio y se coordinara con el
								venderdor via whatsApp
							</p>
						</div>
					</div>
				</section>
